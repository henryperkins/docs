...
class APIHandler:
        ...
        while attempt < self.config.max_retries:
            try:
                ...
                async with self.semaphore:
                    ...
                    self.metrics_manager.record_api_call(provider, latency, tokens_used, success=True)
            ...
            if should_retry and attempt < self.config.max_retries - 1:
                ...
                logger.warning(f"Retry {attempt}/{self.config.max_retries} after {delay}s. Error: {type(last_error).__name__}")
                await asyncio.sleep(delay)
                continue
            else:
                logger.error(f"API request failed: {type(last_error).__name__}")
                break

        processing_time = (datetime.now() - start_time).total_seconds()
        self.metrics_manager.record_api_call(provider, processing_time, 0, success=False, error_type=str(last_error))
    ...
    async def _fetch_completion_generic(self, prompt: List[Dict[str, str]], endpoint: str, deployment_name: str, api_key: str, api_version: str) -> Dict[str, Any]:
        ...
        payload = {"messages": prompt, "max_tokens": self.config.max_tokens, "temperature": self.config.temperature}
        ...
        async with self._rate_limit_lock:
            if provider in self._rate_limit_tokens:
                ...
                if tokens <= 0 and datetime.now() < reset_time:
                    ...
                    logger.warning(f"Rate limit reached for {provider}. Waiting {wait_time:.1f}s")
...
class FileProcessor:
    ...
    def __init__(self, context_manager: HierarchicalContextManager, api_handler: APIHandler, provider_config: ProviderConfig, metrics_manager: MetricsManager):
        ...
        self.metrics_manager = metrics_manager
        self.chunk_manager = ChunkManager(max_tokens=provider_config.max_tokens, overlap=provider_config.chunk_overlap)
    ...
    async def process_file(self, file_path: str, skip_types: Set[str], project_info: str, style_guidelines: str, repo_root: str, output_dir: str, provider: str, project_id: str, safe_mode: bool = False) -> ProcessingResult:
        ...
        try:
            ...
            chunks = self.chunk_manager.create_chunks(content, file_path, language)
    ...
    async def _process_chunks(self, chunks: List[CodeChunk], project_info: str, style_guidelines: str, provider: str) -> List[ProcessingResult]:
        ...
        for i in range(0, len(chunks), max_parallel_chunks):
            ...
            tasks = [self._process_chunk(chunk, project_info, style_guidelines, provider) for chunk in chunk_group]
            ...
            for chunk, result in zip(chunk_group, group_results):
                if isinstance(result, Exception):
                    ...
                    results.append(ProcessingResult(success=False, error=str(result)))
                else:
                    ...
                    if result.success and result.content:
                        try:
                            ...
                            logger.warning(f"Failed to store chunk result: {str(e)}")
    ...
    def _build_prompt(self, chunk: CodeChunk, project_info: str, style_guidelines: str) -> List[Dict[str, str]]:
        ...
        prompt = [{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": f"Please generate documentation for the following code:\n\n{chunk.chunk_content}"}]
        if project_info:
