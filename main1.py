...
async def main():
    ...
    parser.add_argument(
        "--model", help="OpenAI model to use (default: gpt-4)", default="gpt-4"
    )
    parser.add_argument(
        "--skip-types",
    ...
    parser.add_argument(
        "--schema", help="Path to function_schema.json", default="function_schema.json"
    )
   ...
   # ---------------------------------------------------------------------
    # Logging of Parameters (Previously marked as "no changes here")
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Repository Path Validation (Previously marked as "no changes here")
    # ---------------------------------------------------------------------
    ...
    if args.skip_types:
        skip_types.update(
            ext.strip() for ext in args.skip_types.split(",") if ext.strip()
        )
        logger.debug(f"Updated skip_types: {skip_types}")

    # ---------------------------------------------------------------------
    # Config Loading (Previously marked as "no changes here")
    # ---------------------------------------------------------------------
    project_info_config = ""
    ...
    if not function_schema_loaded:
        logger.critical(
            f"Failed to load function schema from '{schema_path}'. Exiting."
        )
        sys.exit(1)
    ...
    # ---------------------------------------------------------------------
    # Output File Initialization (Previously marked as "no changes here")
    # ---------------------------------------------------------------------
