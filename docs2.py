...
async def process_file(session: aiohttp.ClientSession, file_path: str, output_file: str, skip_types: List[str]) -> None:
    """
    ...
    if ext in skip_types:
        logging.info(f"Skipping file: {file_path} (type {ext})")

        # Check if the file is binary
        if is_binary(file_path):
            logging.warning(f"Skipped binary file: {file_path}")
            description = f"### Binary File\n**Warning:** {file_path} is a binary file and was skipped."
            content = "**Binary content is not displayed.**"
        else:
            ...
            try:
                ...
                logging.debug(f"Extracted description for skipped file: {file_path}")
            except UnicodeDecodeError as e:
                logging.error(f"UnicodeDecodeError for file '{file_path}': {e}")
                description = f"### File Skipping\n**Error:** Could not decode '{file_path}' as UTF-8."
                content = "**Unable to display content due to encoding issues.**"
            except (FileNotFoundError, IOError) as e:
            ...
            async with OUTPUT_LOCK:
                with open(output_file, 'a', encoding='utf-8') as md_file:
                    ...
                    if ext == '.md':
                        ...
                        # For other skipped types, include the content in a code block
                        language = get_language(ext)
                        md_file.write(f"```{language}\n{content}\n```\n\n")
                    if description:
                        md_file.write(f"{description}\n\n")
            logging.info(f"Documentation for skipped file '{file_path}' written to {output_file}")
        except (FileNotFoundError, IOError) as e:
    ...
    try:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='replace') as content_file:
                content = content_file.read()
            description = f"### Encoding Issue\n**Warning:** Content of '{file_path}' has encoding issues and may be incomplete."
            logging.debug(f"Read file with replacement characters: {file_path}")
        except Exception as ex:
            logging.error(f"Failed to read file '{file_path}' with replacement characters: {ex}")
            description = f"### File Skipping\n**Error:** Could not read '{file_path}' due to encoding issues."
            content = "**Unable to display content due to encoding issues.**"
    except (FileNotFoundError, IOError) as e:
        logging.error(f"Error reading file '{file_path}': {e}")
        description = f"### File Skipping\n**Error:** Could not read '{file_path}'."
    ...
    if not is_binary(file_path):
        language = get_language(ext)
        logging.info(f"Generating documentation for file: {file_path}")
        enhanced_documentation = await fetch_openai(session, content, language)
        if enhanced_documentation:
            logging.debug(f"Received enhanced documentation for file: {file_path}")
        refined_documentation = await fetch_refined_documentation(session, enhanced_documentation)
        if refined_documentation:
            logging.debug(f"Received refined documentation for file: {file_path}")
    else:
        logging.warning(f"Binary file detected during processing (unexpected): {file_path}")
        refined_documentation = "### Binary File\n**Warning:** Binary files are not processed."
        ...
        async with OUTPUT_LOCK:
            with open(output_file, 'a', encoding='utf-8') as md_file:
                ...
                if not is_binary(file_path):
                    ...
                    md_file.write(f"```binary\n{content}\n```\n\n")
                md_file.write(f"### Generated Documentation:\n{refined_documentation}\n\n")
        logging.info(f"Documentation for '{file_path}' written to {output_file}")
    except (FileNotFoundError, IOError) as e:
        logging.error(f"Error writing to output file '{output_file}': {e}")
